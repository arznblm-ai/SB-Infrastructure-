...
- `references/awesome-design-md/index.md` (добавлено 2026-09-02) — библиотека из 74 `DESIGN.md` реальных брендов (формат Google Stitch: токены цветов/типографики/радиусов + правила). Использовать как референс направления при поиске визуального языка и как формат для собственных бренд-спек: один `DESIGN.md` в корне проекта — контракт для агентной вёрстки. Вход — через `index.md` папки (кластеры по индустриям), читать один файл за раз. Источник: https://github.com/VoltAgent/awesome-design-md (MIT)
- `system/skills/taste/SKILL.md` (добавлено 2026-09-13, `/taste`) — Taste Skill v2 (Leonxlnx, MIT): anti-slop режим генерации лендингов/портфолио/редизайнов — design read по брифу, ручки VARIANCE/MOTION/DENSITY, запрет LLM-дефолтов, pre-flight чек; варианты направлений в `variants/`. Читать при вёрстке лендинга после `frontend-design`; дизайн-система проекта важнее дефолтов скилла
