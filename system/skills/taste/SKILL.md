---
name: taste
description: "Taste Skill v2 (Leonxlnx/taste-skill, MIT, ★86k) — anti-slop фронтенд-скилл для лендингов, портфолио и редизайнов: читает бриф, выводит design read, крутит три ручки VARIANCE / MOTION / DENSITY, запрещает LLM-дефолты (AI-фиолетовые градиенты, три одинаковые карточки, Inter+slate-900), pre-flight чек перед сдачей. Use when Антон просит «сделай сайт/лендинг, чтобы не выглядел как AI», «убери generic AI look», «со вкусом», «как на Awwwards», редизайн существующего сайта или вызывает /taste. Варианты (минимализм, брутализм, soft, redesign, stitch DESIGN.md, image-to-code, imagegen, brandkit) — в variants/."
model: inherit
---

# Taste — anti-slop фронтенд

### [[2026-09-13]]

Установлен 2026-09-13 из рилса Nate Herk (связка: Emil Kowalski design skill + Impeccable + Taste + Figma MCP + Playwright). Источник — https://github.com/Leonxlnx/taste-skill, снимок `main`, лицензия MIT (файл рядом). Ниже — оригинальный `skills/taste-skill/SKILL.md` (v2, install name `design-taste-frontend`) без правок; обновление: скачать tarball `main`, заменить тело этого файла ниже черты и папку `variants/`.

**Как это сочетается с остальным в vault:** `design department/references/frontend-design` и `ui-ux-pro-max` — принципы и QA-чеклист; `liquidglass` — визуальный слой (стекло/градиенты); **taste** — режим генерации самой страницы: бриф → design read → код. Дизайн-система проекта (Subtle Gradient / DESIGN.md) имеет приоритет над эстетическими дефолтами скилла, как и слова Антона.

**Варианты в `variants/`** (каждый — самостоятельный SKILL.md, подключать при выбранном направлении): `taste-skill-v1` (прежнее поведение), `gpt-tasteskill` (строже, для Codex), `minimalist-skill` (Notion/Linear), `soft-skill` (дорогой спокойный UI), `brutalist-skill`, `redesign-skill` (аудит существующего сайта), `output-skill` (против полусделанной работы), `stitch-skill` (+ формат DESIGN.md — совместим с нашей библиотекой дизайн-систем), `image-to-code-skill`, `imagegen-frontend-web` / `-mobile`, `brandkit` (только картинки-референсы).
