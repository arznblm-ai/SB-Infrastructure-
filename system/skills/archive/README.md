...
## Выведено 2026-09-10 (решение Антона)

| Скилл | Почему выведен |
|---|---|
| `sema` | Антон: «Сема странный агент». Персона не нужна; карьерная информация хранится в одном файле `context/{self} {research} карьерный статус – 2026-09-10.md`, который главная сессия читает и обновляет при карьерных событиях. Папка `infrastructure/Career Strategist/` не перемещена: в ней живёт радар вакансий (LaunchAgent). Обёртка `_commands/sema.md` — здесь же. |
