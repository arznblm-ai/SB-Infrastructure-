...
def build_batch_prompt(chats, memory_dir=None) -> str:
    ...
    return "\n\n".join(
        [
            ...
            (
                "Что сделать.\n"
                "1. Обнови память проектов в %s: кто что взял, сроки, сдачи, риски. "
                "Особенно фиксируй поручения из сообщений владельца вида "
                "\"Егор, жду стилы к 19:00\" - кому, что, к какому сроку.\n"
                "2. Соответствие chat_id и проекта ведёшь сама там же в памяти. "
                "Незнакомый chat_id - запиши как неопознанный и при случае спроси "
                "владельца, что это за проект.\n"
                "3. Реши, есть ли что-то, о чём владельцу нужно знать прямо сейчас: "
                "сорванный или горящий срок, блокер, вопрос лично к нему, "
                "договорённость, которая расходится с планом. Мелкую текучку не "
                "приносишь." % mem
            ),
            (
                "Ответ. Если есть что сказать - верни ТОЛЬКО текст сообщения "
                "владельцу, коротко. Если сказать нечего - верни ровно одно слово "
...
def build_evening_prompt(memory_dir=None) -> str:
    ...
    return "\n\n".join(
        [
            ...
            (
                "Только проблемы, просрочки и то, что ждёт его решения. "
                "Что идёт нормально - не перечисляешь. Не выдумывай фактов: "
                "чего нет в памяти, того не утверждай."
            ),
